...
function test_envdir_suffix() {
    docker_exec "$1" "cat /run/etc/wal-e.d/env/WALG_S3_PREFIX"
    docker_exec "$1" "cat /run/etc/wal-e.d/env/WALE_S3_PREFIX"
...
# TEST SUITE 1 - In-place major upgrade 10->11->...->14
# TEST SUITE 2 - Major upgrade 10->14 after wal-e clone (with CLONE_PGVERSION set)
...
function test_spilo() {
    # TEST SUITE 1
    local container=$1
    log_info "$container is the leader"
    ...
    upgrade3_container=$(start_clone_with_wale_upgrade_to_14_container) # SCOPE=upgrade3 PGVERSION=14 CLONE: _SCOPE=demo _PGVERSION=10 _TARGET_TIME=<next_min>
    log_info "[TS2] Started $upgrade3_container for testing major upgrade 10->14 after clone with wal-e"
    ...
    create_timescaledb "$container"  # we don't install it at the beginning, as we do 10->14 in a clone
    ...
    # TEST SUITE 2
    log_info "[TS2] Testing in-place major upgrade 10->14 after wal-e clone"
    run_test verify_clone_upgrade "$upgrade3_container" "wal-e" 10 14
    ...
    clone14_container=$(start_clone_with_wale_14_container) # SCOPE=clone14 CLONE: _SCOPE=upgrade3 _PGVERSION=14 _TARGET_TIME=<next_hour>
    log_info "[TS3] Started $clone14_container for testing point-in-time recovery (clone with wal-e) with unreachable target on 13+"
    ...
    log_info "[TS1] Testing in-place major upgrade 14->14"
    run_test test_successful_inplace_upgrade_to_14 "$container"
    wait_all_streaming "$container"
    run_test test_envdir_updated_to_x 14
